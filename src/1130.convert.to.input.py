...
def parse_reasoning_steps(text: str):
    ...
    for step_num, block in matches:
        reasoning_match = re.search(
            r"Reasoning:\s*(.*?)(?:- Candidates:|$)", block, flags=re.DOTALL
        )
        ...
        if candidates_match:
            candidates_raw = candidates_match.group(1)
            candidates = re.findall(r"\b[A-E]\b", candidates_raw)
            candidates = sorted(set(candidates), key=lambda x: "ABCDE".index(x))
